...
public class TweetParser {
	...
	static String extractColumn(String csvLine, int csvColumn) {
		...
		if (csvColumn < 0 || csvLine == null || csvColumn > columns.length) {
			return null;
		}
		
		return columns[csvColumn];
	}
	...
	static List<String> parseAndCleanSentence(String sentence) {
		...
		for (int i = 0; i < word.length; i++) {
			word[i] = TweetParser.cleanWord(word[i]);
			if (word[i] != null && !word[i].contains(URL_REGEX)) {
				wordList.add(word[i]);
			}
		}
		return wordList;
	...
	static List<List<String>> parseAndCleanTweet(String tweet) {
		String tweet2 = TweetParser.removeURLs(tweet);
		List<String> sentenceList = TweetParser.sentenceSplit(tweet2);
		List<String> holder = new LinkedList<String>();
		List<List<String>> cleanedTweet = new LinkedList<List<String>>();
		for (String sentence : sentenceList) {
			holder = TweetParser.parseAndCleanSentence(sentence);
			if (holder != null) {
				cleanedTweet.add(holder);
			}
		}
		return cleanedTweet;
	}
	...
	static List<String> csvFileToTweets(String pathToCSVFile, int tweetColumn) {
		if (pathToCSVFile.equals(null)) {
			throw new IllegalArgumentException();
		}
		FileLineIterator li = new FileLineIterator(pathToCSVFile);
		List<String> extractedTweets = new LinkedList<String>();
		while (li.hasNext()) {
			String line = li.next();
			String extractedTweet = TweetParser.extractColumn(line, tweetColumn);
	...
	public static List<List<String>> csvFileToTrainingData(String pathToCSVFile, int tweetColumn) {
		List<String> extractedTweets = csvFileToTweets(pathToCSVFile, tweetColumn);
		List<List<String>> cleanedTweet = new LinkedList<List<String>>();
		List<List<String>> trainingData = new LinkedList<List<String>>();
		for (String tweet : extractedTweets) {
			if (tweet == null) {
				continue;
			}
			cleanedTweet = parseAndCleanTweet(tweet);
			for (List<String> sentence : cleanedTweet) {
