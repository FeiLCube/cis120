...
public class TwitterBot {
	...
	public TwitterBot(String csvFile, int tweetColumn, NumberGenerator ng) {
		...
		this.ng = ng;
		List<List<String>> trainingData = TweetParser.csvFileToTrainingData(csvFile, tweetColumn);
		for (List<String> sentence : trainingData) {
			mc.train(sentence.iterator());
		}
	}
	 ...
	 *                       previous contents
	 */
	public void writeStringsToFile(List<String> stringsToWrite, String filePath, boolean append) throws IOException {
		File file = Paths.get(filePath).toFile();
		if (!file.exists()) {
			try {
				File newFile = File.createTempFile("prefix", "suffix", file);
				file = newFile;
			} catch (IOException e) {
				System.out.println("IOException caught while creating new file.");
			}
		}
		...
		for (String string : stringsToWrite) {
			try {
				...
				br.close();
				System.out.println("IOException caught in while writing to file.");
			}
		}
	...
	public String generateTweet(int length) {
		...
		while (generatedTweet.length() < length) {
			/*if (firstSentence) {
				firstSentence = false;
				mc.reset();
			}*/
			...
			if (mc.hasNext()) {
				generatedTweet += mc.next();
				if (generatedTweet.length() + 1 >= length) {
					generatedTweet += randomPunctuation();
				} else if (!mc.hasNext() && generatedTweet.length() + 1 < length) {
					continue;
				} else {
					generatedTweet += " ";
				}
				...
				if (sentenceNumber == 0) {
					...
					generatedTweet += randomPunctuation();
					if (generatedTweet.length() < length) {
						generatedTweet += " ";
					}
				}
			}
