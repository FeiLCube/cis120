...
public class MarkovChain implements Iterator<String> {
	...
	void addBigram(String first, String second) {
		...
		if (this.chain.containsKey(first)) {
			this.chain.get(first).record(second);
		} else {
			this.chain.put(first, new ProbabilityDistribution<String>());
			this.chain.get(first).record(second);
		}
	}
	...
	public void train(Iterator<String> sentence) {
		...
		while (sentence.hasNext()) {
			if (i == 0) {
				next2 = sentence.next();
				startWords.record(next2);
				i++;
			} else {
				curr2 = next2;
				next2 = sentence.next();
				addBigram(curr2, next2);
				if (!sentence.hasNext()) {
					addBigram(next2, null);
				}
				i++;
			}
		}
